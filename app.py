...
# Custom CSS for UI and Spec Cards
st.markdown("""
    <style>
...
@st.cache_data(show_spinner="📥 Cleaning and Syncing Data...")
    ...
    try:
        ...
        for col in clean_cols:
            if col in df.columns:
                # Convert to string, lowercase, and filter
...
if page == "🏠 Dashboard Home":
    ...
    grouped = b_df.groupby('model').agg({
    ...
    with t1:
        ...
        # Ensure statsmodels is in requirements.txt
    ...
    with t2:
        ...
        # FIXED SUNBURST: Using specific path and cleaned data
        sun_df = df.dropna(subset=['drive', 'type']).sample(min(len(df), 1500))
        fig_sun = px.sunburst(sun_df, path=['type', 'drive'], values='price',
    ...
    brand_m = st.selectbox("Filter Map:", sorted(df['manufacturer'].unique()))
